...
public class PermissionReq {
    ...
    public interface Result {
        void onGranted(String pe);

        void onDenied(String pe);
    }
    ...
    public void request() {
        ...
        for (String permission : mPermissions) {
            if (!sManifestPermissionSet.contains(permission)) {
                if (mResult != null) {
                    mResult.onDenied(permission);
                }
                return;
            }else {
                //这样可以使之前有申请的也发送通知
                if (mResult != null) {
                    mResult.onGranted(permission);
                }
            }
        }
    ...
    public static void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        for (int i=0;i<grantResults.length;i++) {
            ...
            if (grantResult != PackageManager.PERMISSION_GRANTED) {
                result.onDenied(permissions[i]);
                return;
            }
        }
        for(String permission:permissions)
        result.onGranted(permission);
    }
